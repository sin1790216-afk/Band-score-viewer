...
- Student의 내 음원은 기기 로컬 파일을 선택해 native audio controls, 지속 재생, 파형 탐색·확대, start offset 이동과 재생속도를 사용할 수 있다. stable measure ID에 정확한 음원 위치를 수동 지정하고 하이라이트 클릭으로 seek할 수 있지만, 재생 시간에 따라 하이라이트가 자동 진행되는 기능은 아직 구현하지 않았다.
