...
- 현재는 Teacher 링크/start offset 공유와 Student의 PDF별 개인 설정을 기록하고 안전한 웹 주소를 새 탭으로 여는 단계다.
- start offset은 아직 음원 재생에 적용되지 않으며, 음원 재생·마디 하이라이트 연동과 재생속도 조절은 구현하지 않았다.
- Student 개인 음원 설정은 브라우저 로컬 저장소에만 있으므로 사이트 데이터를 지우면 사라지고 다른 기기로 동기화되지 않는다.
